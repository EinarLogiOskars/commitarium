...
// EnsurePullRequestRevision appends one compact audit entry after the exact
// commit is visible as the pull request head. The marker makes a retry after an
// uncertain PATCH response idempotent.
func (client *Client) EnsurePullRequestRevision(
	ctx context.Context,
	owner string,
	repository string,
	...
	if stored.Number != spec.Number || stored.State != "open" || !stored.Draft ||
		stored.BaseBranch != spec.BaseBranch || stored.HeadBranch != spec.HeadBranch ||
		stored.HeadCommitID != spec.CommitID ||
		!strings.Contains(stored.Body, spec.FeatureMarker) {
		return workspace.PullRequest{}, false, workspace.ErrPullRequestConflict
	}
	section := spec.PublicationMarker + "\n\n## Implementation revision\n\n" +
		"`" + spec.CommitID + "` — " + spec.CommitMessage
	if count := strings.Count(stored.Body, spec.PublicationMarker); count > 0 {
		if count != 1 || !strings.Contains(stored.Body, section) {
			return workspace.PullRequest{}, false, workspace.ErrPullRequestConflict
		}
	...
	if status == http.StatusNotFound || status == http.StatusForbidden || status == http.StatusLocked {
	...
	if updated.Number != spec.Number || updated.HeadCommitID != spec.CommitID ||
